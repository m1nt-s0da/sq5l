...
```python
table("users").where(lambda users: users.age >= param(20))
```
...
```python
.order(("age", "asc"), (lambda users: users.created_at + param(1), "desc"))
```
...
注意: limit が None の場合、OFFSET/LIMIT 句は出力されません。
...
.group_by(
    "user_id",
    having=lambda payments: (asterisk.count() >= param(3)) & (payments.amount.avg() >= param(2000)),
)
```
...
.where(lambda users: users.id.in_(
    table("orders").where(lambda orders: orders.total >= param(3000)).select(lambda orders: orders.user_id)
))
```
...
q, p = sub.where(lambda t: t.user_id >= param(1)).select(lambda t: t.user_id).query()
```
